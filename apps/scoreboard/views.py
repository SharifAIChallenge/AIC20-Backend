...
from .serializers import ScoreBoardSerializer, RowSerializer
...
class GroupScoreBoardAPIView(GenericAPIView):
    ...
    def get(self, request, group_id):
        ...
        data = self.get_serializer(GroupScoreBoard.get_scoreboard_sorted_rows(group=group), many=True).data
        return Response(data={'scoreboard': data}, status=status.HTTP_200_OK)
